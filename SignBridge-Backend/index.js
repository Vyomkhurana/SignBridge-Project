...
wss.on('connection', (ws) => {
    ...
    ws.on('message', async (message) => {
        try {
            ...
            const recognizedWord = mlResponse.data.text;

            if (recognizedWord && recognizedWord !== user.lastLetter) {
                user.currentWord = recognizedWord; // For words, replace instead of append
                user.lastLetter = recognizedWord;
                console.log(`User ${userId} detected word: "${recognizedWord}"`);

                // Send live text update immediately
                ws.send(JSON.stringify({ type: 'live_text', data: recognizedWord }));
                ...
                user.speakTimeout = setTimeout(async () => {
                    ...
                    console.log(`User ${userId} speaking word: "${wordToSpeak}"`);
            ...
            } else if (!recognizedWord) {
                // Send status update for no detection
            ...
            ws.send(JSON.stringify({ type: 'error', data: 'Failed to process frame.' }));
        }
    });
...
server.listen(PORT, () => console.log(`🚀 SignBridge Backend running on port ${PORT}`));
